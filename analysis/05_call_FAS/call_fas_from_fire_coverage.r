...
paramerer_summary <- paste0("bin",bin_size,"_smooth",smooth_k,"_outCDRCutoff",out_CDR_cutoff,"_inCDRCutoff",in_CDR_cutoff,"_cdrExp",cdr_expansion)
...
p_stat <- ggplot(stat_long, aes(cell, value, fill = cell)) +
  ...
  labs(x = paramerer_summary, y = "Value", title = "FAS comparison across cell types") +
