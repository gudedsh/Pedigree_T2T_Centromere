...
if (is.null(in_file) || is.null(cen_file)) {
  stop("Usage: Rscript call_cars.r --input bin_summary.tsv --cen activeHOR_CDR.tsv")
}
if (!file.exists(in_file)) stop("Bin-summary input not found: ", in_file)
...
if (!is.finite(out_CDR_cutoff) || out_CDR_cutoff < 0 || out_CDR_cutoff >= 0.5) stop("--out_CDR_cutoff must be in [0, 0.5)")
if (!is.finite(in_CDR_cutoff) || in_CDR_cutoff < 0 || in_CDR_cutoff >= 0.5) stop("--in_CDR_cutoff must be in [0, 0.5)")

dir.create(plotdir, recursive = TRUE, showWarnings = FALSE)

out_summary <- file.path(plotdir, "results_summary.tsv")
...
if (!nrow(dt)) stop("No finite values remain in CARS signal column: ", cars_col)
...
if (!length(cen_lines)) stop("Active-HOR/CDR annotation contains no intervals")
...
if(any(is.na(cen$active_start) | is.na(cen$active_end) | is.na(cen$cdr_start_raw) | is.na(cen$cdr_end_raw) |
       cen$active_start < 0 | cen$active_end <= cen$active_start |
       cen$cdr_start_raw < cen$active_start | cen$cdr_end_raw > cen$active_end |
       cen$cdr_end_raw <= cen$cdr_start_raw)) stop("Invalid active-HOR/CDR coordinates.")
...
plot_one_chr <- function(plot_dt,cr,res_chr){
  ...
  p1 <- ggplot()+
    ...
    labs(x=NULL,y="Normalized CENP-A signal",title=paste0(cr$chr,": CARS / CENP-A enrichment"))+
...
for(i in seq_len(nrow(cen))){
  ...
  for(sid in unique(chr_dt$sample)){
    ...
    plot_list[[paste0(cr$chr, ":", cr$active_start, "-", cr$active_end)]] <- p
}

## output results
if(!nrow(res_dt)) stop("No CARS results were produced.")
preferred_columns <- c(
  "sample", "chr", "CARS", "log2_CARS", "Fold_Enrichment",
  "bg_signal", "cdr_signal", "bg_signal_raw", "cdr_signal_raw",
  "n_active_bins", "n_bg_bins_raw", "n_cdr_bins_raw"
)
preferred_columns <- intersect(preferred_columns, names(res_dt))
setcolorder(res_dt, c(preferred_columns, setdiff(names(res_dt), preferred_columns)))
